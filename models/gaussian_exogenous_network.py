...
class GaussianExogenousNetwork(nn.Module):
    def __init__(self, history_size, feature_size, hidden_size, hidden_size_history, output_size):
        ...
        module_list_base = [nn.ReLU(),
                            nn.Linear(self.hidden_size[0] * 9 + self.hidden_size_history, self.hidden_size[1]),
                            nn.ReLU(),
    ...
    def forward(self, history, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin, feature1, feature2,
        ...
        x = torch.cat(
            [history, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin, feature1, feature2, feature3], dim=1)
...
def train_exogenous_gaussian_model(model, history, target, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin,
                                   feature1, feature2,
                                   feature3, num_epochs, batch_size, optimizer, save_path, save_name):
    model.train()
    train_losses = []
...
def create_exogenous_gaussian_predictions(model, history, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin,
                                          feature1, feature2,
                                          feature3):
    model.eval()
    with torch.no_grad():
