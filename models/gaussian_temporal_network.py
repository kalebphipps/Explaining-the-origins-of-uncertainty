...
class GaussianTemporalNetwork(nn.Module):
    def __init__(self, history_size, temporal_size, hidden_size, hidden_size_history, output_size):
        super().__init__()
        ...
        self.hour_cos_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))

        self.hour_sin_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))

        self.day_cos_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))

        self.day_sin_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))

        self.month_cos_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))

        self.month_sin_network = nn.Sequential(nn.Linear(self.temporal_size, self.hidden_size[0]))
...
def train_gaussian_temporal_model(model, history, target, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin,
...
def create_gaussian_temporal_predictions(model, history, hour_cos, hour_sin, day_cos, day_sin, month_cos, month_sin):
    model.eval()
    with torch.no_grad():
