...
class GaussianSmallExogenousNoTemporalNetwork(nn.Module):
...
def train_small_exogenous_no_temp_gaussian_model(model, history, target, feature1, feature2, num_epochs, batch_size,
...
def create_small_exogenous_no_temp_gaussian_predictions(model, history, feature1, feature2):
    model.eval()
    with torch.no_grad():
